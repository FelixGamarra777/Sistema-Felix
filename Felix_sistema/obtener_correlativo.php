...
try {
    if ($tipo === 'egreso') {
        // Correlativo de egresos con prefijo EGR- (namespace separado del de
        // ingresos, así ninguno de los dos pisa al otro).
        $stmt = $pdo->query("
            SELECT COALESCE(MAX(CAST(SUBSTRING(numero_factura, 5) AS UNSIGNED)), 0) + 1 AS siguiente
            FROM facturas
            WHERE numero_factura REGEXP '^EGR-[0-9]+$'
        ");
        $siguiente = (int)$stmt->fetchColumn();
        echo json_encode(["exito" => true, "numero_factura" => 'EGR-' . str_pad($siguiente, 5, '0', STR_PAD_LEFT)]);
    } else {
        // Correlativo de ingresos: solo números puros (excluye los EGR-).
        $stmt = $pdo->query("
